...
class DDPG(object):
  ...
  def __init__(self,
    ...
    with tf.compat.v1.variable_scope('actor'):
      ...
      self.actor_lr = tf.Variable(actor_lr, name='lr')
      self.actor_step = tf.Variable(
          0, dtype=tf.int64, name='step')
      self.actor_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=self.actor_lr)
    ...
    with tf.compat.v1.variable_scope('critic'):
      ...
      self.critic_step = tf.Variable(
          0, dtype=tf.int64, name='step')
      self.critic_optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=critic_lr)
  ...
  def _update_critic_ddpg(self, obs, action, next_obs, reward, mask):
    ...
    with tf.GradientTape() as tape:
      ...
      critic_loss = tf.compat.v1.losses.mean_squared_error(q_target, q_pred)

    grads = tape.gradient(critic_loss, self.critic.variables)
    self.critic_optimizer.apply_gradients(
        zip(grads, self.critic.variables), global_step=self.critic_step)

    with tf.summary.record_if(self.critic_step % 100 == 0):
      tf.compat.v2.summary.scalar('critic/loss', critic_loss, step=self.critic_step)
  ...
  def _update_critic_td3(self, obs, action, next_obs, reward, mask):
    ...
    target_action_noise = tf.Variable(target_action_noise)
    ...
    with tf.GradientTape() as tape:
      ...
      critic_loss = tf.compat.v1.losses.mean_squared_error(
          q_target, q_pred1) + tf.compat.v1.losses.mean_squared_error(
              q_target, q_pred2)
  ...
  def _update_actor(self, obs, mask):
    ...
    with tf.summary.record_if(self.actor_step % 100 == 0):
      tf.compat.v2.summary.scalar('actor/loss', actor_loss, step=self.actor_step)
