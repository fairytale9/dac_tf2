...
def main(_):
  ...
  gym_random_state_var = tf.Variable(
      '', name='gym_random_state')
  np_random_state_var = tf.Variable(
      '', name='np_random_state')
  py_random_state_var = tf.Variable(
      '', name='py_random_state')

  reward_scale = tf.Variable(1, name='reward_scale')
  ...
  eval_saver = tf.train.Checkpoint(model.actor.variables +
                                              [reward_scale])
  tf.io.gfile.makedirs(FLAGS.eval_save_dir)

  last_checkpoint = tf.train.latest_checkpoint(FLAGS.save_dir)
  if last_checkpoint is None:
    expert_saver = tf.train.Checkpoint([expert_replay_buffer_var])
    ...
    while total_numsteps < FLAGS.training_steps:
      ...
      with tf.compat.v2.summary.record_if(True):
        tf.compat.v2.summary.scalar(
            ...
            step=total_numsteps)
        tf.compat.v2.summary.scalar('reward', rollout_reward, step=total_numsteps)
        tf.compat.v2.summary.scalar('length', rollout_timesteps, step=total_numsteps)
