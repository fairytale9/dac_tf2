...
from tensorflow.python.gan.losses import losses_impl as gan_losses_impl
  ...
  def update(self, batch, expert_batch):
    ...
    with tf.GradientTape() as tape:
      ...
      with tf.summary.record_if(self.disc_step % 100 == 0):
        gan_loss = gan_losses_impl.modified_discriminator_loss(
            expert_output,
            output,
            label_smoothing=0.0,
            real_weights=expert_weight) # not sure
        tf.summary.scalar(
